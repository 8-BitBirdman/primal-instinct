#!/usr/bin/env python3
"""Local verification runner for primal install surfaces."""
...
def verify_skill_frontmatter_upload_compatibility() -> None:
    ...
    skill_paths = [
        ROOT / "skills/primal/SKILL.md",
        ROOT / "skills/primal-commit/SKILL.md",
        ROOT / "skills/primal-help/SKILL.md",
        ROOT / "skills/primal-review/SKILL.md",
        ROOT / "primal-compress/SKILL.md",
    ]
...
def verify_synced_files() -> None:
    ...
    skill_source = ROOT / "skills/primal/SKILL.md"
    rule_source = ROOT / "rules/primal-activate.md"

    skill_copies = [
        ROOT / "primal/SKILL.md",
        ROOT / "plugins/primal/skills/primal/SKILL.md",
        ROOT / ".cursor/skills/primal/SKILL.md",
        ROOT / ".windsurf/skills/primal/SKILL.md",
    ]
    ...
    with zipfile.ZipFile(ROOT / "primal.skill") as archive:
        ensure("primal/SKILL.md" in archive.namelist(), "primal.skill missing primal/SKILL.md")
        ensure(
            archive.read("primal/SKILL.md").decode("utf-8")
            == skill_source.read_text(encoding="utf-8"),
            "primal.skill payload mismatch",
        )

    print("Synced copies and primal.skill zip OK")
...
def verify_manifests_and_syntax() -> None:
    ...
    manifest_paths = [
        ...
        ROOT / "plugins/primal/.codex-plugin/plugin.json",
    ]
    for path in manifest_paths:
        read_json(path)

    run(["node", "--check", "hooks/primal-config.js"])
    run(["node", "--check", "hooks/primal-activate.js"])
    run(["node", "--check", "hooks/primal-mode-tracker.js"])
    run(["bash", "-n", "hooks/install.sh"])
    run(["bash", "-n", "hooks/uninstall.sh"])
    run(["bash", "-n", "hooks/primal-statusline.sh"])

    # Ensure install/uninstall scripts include primal-config.js
    install_sh = (ROOT / "hooks/install.sh").read_text(encoding="utf-8")
    uninstall_sh = (ROOT / "hooks/uninstall.sh").read_text(encoding="utf-8")
    ensure("primal-config.js" in install_sh, "install.sh missing primal-config.js")
    ensure("primal-config.js" in uninstall_sh, "uninstall.sh missing primal-config.js")
...
def verify_powershell_static() -> None:
    ...
    statusline_text = (ROOT / "hooks/primal-statusline.ps1").read_text(encoding="utf-8")

    ensure("primal-config.js" in install_text, "install.ps1 missing primal-config.js")
    ensure("primal-config.js" in uninstall_text, "uninstall.ps1 missing primal-config.js")
    ensure("primal-statusline.ps1" in install_text, "install.ps1 missing statusline.ps1")
    ensure("primal-statusline.ps1" in uninstall_text, "uninstall.ps1 missing statusline.ps1")
    ...
    ensure("[PRIMAL" in statusline_text, "primal-statusline.ps1 missing badge output")

    print("Windows install path statically wired")


def load_compress_modules():
    sys.path.insert(0, str(ROOT / "primal-compress"))
...
def verify_compress_fixtures() -> None:
    ...
    fixtures = sorted((ROOT / "tests/primal-compress").glob("*.original.md"))
    ensure(fixtures, "No primal-compress fixtures found")
    ...
    print(f"Validated {len(fixtures)} primal-compress fixture pairs")
...
def verify_hook_install_flow() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="primal-verify-") as temp_root:
        ...
        activate = run(
            ["node", "hooks/primal-activate.js"],
            env=hook_env,
        )
        ensure("PRIMAL MODE ACTIVE" in activate.stdout, "activation output missing primal banner")
        ensure("STATUSLINE SETUP NEEDED" not in activate.stdout, "activation should stay quiet when custom statusline exists")
        ensure((claude_dir / ".primal-active").read_text(encoding="utf-8") == "full", "activation flag should default to full")

        # Test configurable default mode via PRIMAL_DEFAULT_MODE env var
        activate_custom = run(
            ["node", "hooks/primal-activate.js"],
            env={**hook_env, "PRIMAL_DEFAULT_MODE": "ultra"},
        )
        ensure("PRIMAL MODE ACTIVE" in activate_custom.stdout, "activation with custom default missing banner")
        ensure(
            (claude_dir / ".primal-active").read_text(encoding="utf-8") == "ultra",
            "PRIMAL_DEFAULT_MODE=ultra should set flag to ultra",
        )
        # Test "off" mode — activation skipped, flag removed
        activate_off = run(
            ["node", "hooks/primal-activate.js"],
            env={**hook_env, "PRIMAL_DEFAULT_MODE": "off"},
        )
        ensure("PRIMAL MODE ACTIVE" not in activate_off.stdout, "off mode should not emit primal banner")
        ensure(not (claude_dir / ".primal-active").exists(), "off mode should remove flag file")

        # Test mode tracker with /primal when default is off — should NOT write flag
        subprocess.run(
            ["node", "hooks/primal-mode-tracker.js"],
            cwd=ROOT,
            env={**os.environ, **hook_env, "PRIMAL_DEFAULT_MODE": "off"},
            text=True,
            encoding="utf-8",
            input='{"prompt":"/primal"}',
            capture_output=True,
        ...
        ensure(not (claude_dir / ".primal-active").exists(), "/primal with off default should not write flag")

        # Reset back to full for subsequent tests
        (claude_dir / ".primal-active").write_text("full")
        ...
        ultra_prompt = subprocess.run(
            ...
            text=True,
            encoding="utf-8",
            input='{"prompt":"/primal ultra"}',
            capture_output=True,
            check=True,
        )
        ensure(
            "PRIMAL MODE ACTIVE (ultra)" in ultra_prompt.stdout,
            "mode tracker should emit active-mode reinforcement",
        )
        ensure((claude_dir / ".primal-active").read_text(encoding="utf-8") == "ultra", "mode tracker did not record ultra")
        ...
        ensure(not (claude_dir / ".primal-active").exists(), "normal mode should remove flag file")

        (claude_dir / ".primal-active").write_text("wenyan-ultra")
        statusline = run(
            ["bash", "hooks/primal-statusline.sh"],
            env=hook_env,
        )
        ensure("[PRIMAL:WENYAN-ULTRA]" in statusline.stdout, "statusline badge output mismatch")
        ...
        ensure(settings_after == existing_settings, "uninstall.sh did not restore non-primal settings")
        ensure(not (claude_dir / ".primal-active").exists(), "uninstall.sh should remove flag file")

    with tempfile.TemporaryDirectory(prefix="primal-verify-fresh-") as temp_root:
        ...
        activate = run(["node", "hooks/primal-activate.js"], env=hook_env)
