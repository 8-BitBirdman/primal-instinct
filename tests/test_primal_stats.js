#!/usr/bin/env node
// Tests for /primal-stats — direct script invocation and via mode tracker.
// Run: node tests/test_primal_stats.js
...
const STATS = path.join(ROOT, 'hooks', 'primal-stats.js');
const TRACKER = path.join(ROOT, 'hooks', 'primal-mode-tracker.js');

let passed = 0;
let failed = 0;

function test(name, fn) {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'primal-stats-test-'));
  try {
    fn(tmp);
    passed++;
    console.log(`  ✓ ${name}`);
...
console.log('primal-stats tests\n');
...
test('shows full-mode savings estimate when flag is full', (tmp) => {
  ...
  fs.writeFileSync(path.join(claudeDir, '.primal-active'), 'full');
  const out = execFileSync(process.execPath, [STATS, '--session-file', sess], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir },
  });
  // 350 / 0.35 = 1000, saved = 650, ~65%
  assert.match(out, /Est\. without primal:\s+1,000/);
...
test('skips estimate for non-full modes', (tmp) => {
  ...
  const claudeDir = path.join(tmp, '.claude');
  fs.writeFileSync(path.join(claudeDir, '.primal-active'), 'ultra');
  const out = execFileSync(process.execPath, [STATS, '--session-file', sess], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir },
  });
...
test('mode tracker handles /primal-stats with decision block', (tmp) => {
  const sess = makeSession(tmp, [
    { type: 'assistant', message: { usage: { output_tokens: 100 } } },
  ]);
  const claudeDir = path.join(tmp, '.claude');
  ...
  const out = execFileSync(process.execPath, [TRACKER], {
    ...
    input: JSON.stringify({ prompt: '/primal-stats', transcript_path: sess }),
  });
  const parsed = JSON.parse(out);
  assert.strictEqual(parsed.decision, 'block');
  assert.match(parsed.reason, /Primal Stats/);
  assert.match(parsed.reason, /Output tokens:\s+100/);
});

test('mode tracker preserves primal flag when /primal-stats fires', (tmp) => {
  const sess = makeSession(tmp, [
  ...
  assert.strictEqual(fs.readFileSync(path.join(claudeDir, '.primal-active'), 'utf8'), 'full');
});
...
test('priceForModel matches by prefix across point releases', () => {
  const { priceForModel } = require(path.join(ROOT, 'hooks', 'primal-stats.js'));
...
test('formatStats handles empty session gracefully', () => {
  const { formatStats } = require(path.join(ROOT, 'hooks', 'primal-stats.js'));
...
test('--share prints single-line tweetable summary', (tmp) => {
  ...
  assert.match(out, /^🪨 Saved 650 output tokens \(~\$0\.009[78]\) across 1 turns this session — primal\.sh$/m);
});
...
test('--share works with no benchmark ratio (lite mode)', (tmp) => {
  ...
  const claudeDir = path.join(tmp, '.claude');
  fs.writeFileSync(path.join(claudeDir, '.primal-active'), 'lite');
  const out = execFileSync(process.execPath, [STATS, '--session-file', sess, '--share'], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir },
  });
  assert.match(out, /^🪨 1 turns, 200 output tokens this session — primal\.sh$/m);
});
...
test('appends to lifetime history on each run', (tmp) => {
  ...
  const histPath = path.join(claudeDir, '.primal-history.jsonl');
...
test('skips pairs where compressed is not actually smaller', (tmp) => {
  const { findCompressedPairs } = require(path.join(ROOT, 'hooks', 'primal-stats.js'));
...
test('writes statusline suffix file after a stats run', (tmp) => {
  ...
  const suffixPath = path.join(claudeDir, '.primal-statusline-suffix');
...
test('humanizeTokens formats small/medium/large correctly', () => {
  const { humanizeTokens } = require(path.join(ROOT, 'hooks', 'primal-stats.js'));
...
test('statusline.sh appends savings when PRIMAL_STATUSLINE_SAVINGS=1', (tmp) => {
  if (process.platform === 'win32') return; // bash test
  const claudeDir = path.join(tmp, '.claude');
  fs.mkdirSync(claudeDir, { recursive: true });
  fs.writeFileSync(path.join(claudeDir, '.primal-active'), 'full');
  fs.writeFileSync(path.join(claudeDir, '.primal-statusline-suffix'), '⛏ 2.8k');
  const out = execFileSync('bash', [path.join(ROOT, 'hooks', 'primal-statusline.sh')], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir, PRIMAL_STATUSLINE_SAVINGS: '1' },
  });
  assert.match(out, /\[PRIMAL\]/);
...
test('statusline.sh renders savings by default when env var is unset', (tmp) => {
  ...
  delete env.PRIMAL_STATUSLINE_SAVINGS;
  const out = execFileSync('bash', [path.join(ROOT, 'hooks', 'primal-statusline.sh')], {
    encoding: 'utf8', env,
  });
  assert.match(out, /\[PRIMAL\]/);
...
test('statusline.sh omits savings when PRIMAL_STATUSLINE_SAVINGS=0', (tmp) => {
  if (process.platform === 'win32') return;
  const claudeDir = path.join(tmp, '.claude');
  fs.mkdirSync(claudeDir, { recursive: true });
  fs.writeFileSync(path.join(claudeDir, '.primal-active'), 'full');
  ...
  const out = execFileSync('bash', [path.join(ROOT, 'hooks', 'primal-statusline.sh')], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir, PRIMAL_STATUSLINE_SAVINGS: '0' },
  });
  assert.match(out, /\[PRIMAL\]/);
...
test('statusline.sh omits savings when suffix file is missing (fresh install)', (tmp) => {
  ...
  // /primal-stats has run. Default-on must NOT fabricate a number.
  const env = { ...process.env, CLAUDE_CONFIG_DIR: claudeDir };
  delete env.PRIMAL_STATUSLINE_SAVINGS;
  const out = execFileSync('bash', [path.join(ROOT, 'hooks', 'primal-statusline.sh')], {
    encoding: 'utf8', env,
...
test('statusline.sh strips control bytes from suffix', (tmp) => {
  ...
  fs.writeFileSync(path.join(claudeDir, '.primal-statusline-suffix'), '\x1b[31mEVIL');
  const out = execFileSync('bash', [path.join(ROOT, 'hooks', 'primal-statusline.sh')], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir, PRIMAL_STATUSLINE_SAVINGS: '1' },
  });
...
test('appendFlag is symlink-safe (refuses symlinked target)', (tmp) => {
  ...
  const { appendFlag } = require(path.join(ROOT, 'hooks', 'primal-config.js'));
...
test('mode tracker forwards --share to stats script', (tmp) => {
  ...
  const out = execFileSync(process.execPath, [TRACKER], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir, HOME: tmp },
    input: JSON.stringify({ prompt: '/primal-stats --share', transcript_path: sess }),
  });
  const parsed = JSON.parse(out);
  assert.strictEqual(parsed.decision, 'block');
