#!/usr/bin/env node
// Tests for tools/primal-init.js — fixture-based.
// Run: node tests/test_primal_init.js
...
const INIT = path.join(ROOT, 'tools', 'primal-init.js');

let passed = 0;
let failed = 0;

function test(name, fn) {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'primal-init-test-'));
  try {
    fn(tmp);
    passed++;
    console.log(`  ✓ ${name}`);
...
console.log('primal-init tests\n');

test('greenfield: creates all rule files with proper frontmatter', (tmp) => {
  execFileSync(process.execPath, [INIT, tmp], { encoding: 'utf8' });
  const cursor = fs.readFileSync(path.join(tmp, '.cursor/rules/primal.mdc'), 'utf8');
  assert.match(cursor, /alwaysApply: true/);
  assert.match(cursor, /Respond terse like smart primal/);
  const windsurf = fs.readFileSync(path.join(tmp, '.windsurf/rules/primal.md'), 'utf8');
  assert.match(windsurf, /trigger: always_on/);
  const cline = fs.readFileSync(path.join(tmp, '.clinerules/primal.md'), 'utf8');
...
test('append mode: existing AGENTS.md gets primal appended (not replaced)', (tmp) => {
  fs.writeFileSync(path.join(tmp, 'AGENTS.md'), '# My project\n\nDo not delete me.\n');
  execFileSync(process.execPath, [INIT, tmp], { encoding: 'utf8' });
  const agents = fs.readFileSync(path.join(tmp, 'AGENTS.md'), 'utf8');
  assert.match(agents, /Do not delete me/);
  assert.match(agents, /Respond terse like smart primal/);
});

test('skip mode: existing .cursor rule is not overwritten without --force', (tmp) => {
  const dir = path.join(tmp, '.cursor/rules');
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, 'primal.mdc'), '# original\nDo not delete me.\n');
  const out = execFileSync(process.execPath, [INIT, tmp], { encoding: 'utf8' });
  assert.match(out, /\? .*\.cursor\/rules\/primal\.mdc/);
  const after = fs.readFileSync(path.join(dir, 'primal.mdc'), 'utf8');
...
test('--only filters to one target', (tmp) => {
  ...
  assert.ok(fs.existsSync(path.join(tmp, '.clinerules/primal.md')));
  assert.ok(!fs.existsSync(path.join(tmp, '.cursor')));
});

test('detects sentinel and skips files that already have primal content', (tmp) => {
  // Hand-write a file that already contains the rule (simulating prior install).
  const dir = path.join(tmp, '.clinerules');
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, 'primal.md'),
    '# Existing\n\nRespond terse like smart primal. Hello.\n');
  const out = execFileSync(process.execPath, [INIT, tmp, '--only', 'cline'], { encoding: 'utf8' });
