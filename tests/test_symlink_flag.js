...
const { safeWriteFlag, readFlag, VALID_MODES } = require('../hooks/primal-config');

let passed = 0;
let failed = 0;

function test(name, fn) {
  const tmpBase = fs.mkdtempSync(path.join(os.tmpdir(), 'primal-symlink-test-'));
  try {
    fn(tmpBase);
    passed++;
    console.log(`  ✓ ${name}`);
...
test('writes flag in normal (non-symlinked) directory', (tmp) => {
  ...
  const flagPath = path.join(flagDir, '.primal-active');
...
test('writes flag when parent directory is a symlink owned by current user', (tmp) => {
  ...
  const flagPath = path.join(symlinkDir, '.primal-active');
  safeWriteFlag(flagPath, 'ultra');

  // Flag should exist in the real directory
  const realFlagPath = path.join(realDir, '.primal-active');
...
test('readFlag works through symlinked parent directory', (tmp) => {
  ...
  const result = readFlag(path.join(symlinkDir, '.primal-active'));
...
test('refuses flag file that is itself a symlink (even through symlinked parent)', (tmp) => {
  ...
  safeWriteFlag(path.join(symlinkDir, '.primal-active'), 'full');
...
test('readFlag refuses flag file that is a symlink', (tmp) => {
  ...
  fs.symlinkSync(secretFile, path.join(realDir, '.primal-active'));

  const result = readFlag(path.join(realDir, '.primal-active'));
...
test('symlink to nonexistent target silently fails', (tmp) => {
  ...
  assert.strictEqual(fs.existsSync(path.join(symlinkDir, '.primal-active')), false);
});
...
test('safeWriteFlag no longer has blanket symlink parent refusal', (tmp) => {
  ...
  const source = fs.readFileSync(
    path.join(__dirname, '..', 'hooks', 'primal-config.js'), 'utf8'
  );
