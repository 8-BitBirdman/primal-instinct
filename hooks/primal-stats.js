#!/usr/bin/env node
// primal-stats — read the active Claude Code session log, print real token
// usage plus an estimated savings figure from the benchmark in benchmarks/.
//
// Run directly:    node hooks/primal-stats.js
// Inside Claude:   /primal-stats triggers this via the UserPromptSubmit hook.
...
const os = require('os');
const { readFlag, appendFlag, readHistory, safeWriteFlag } = require('./primal-config');
...
// Detect *.original.md / *.md pairs left behind by primal-compress. The
...
function formatHistory({ sessions, outputTokens, estSavedTokens, estSavedUsd, since }) {
  ...
  if (sessions === 0) {
    return `\nPrimal Stats — Lifetime${window}\n${sep}\nNo sessions logged yet — run /primal-stats inside any session to start tracking.\n${sep}\n`;
  }
...
function formatShare({ outputTokens, turns, mode, model }) {
  if (turns === 0) {
    return '🥩 primal armed but no turns yet — primal.sh';
  }
  const ratio = COMPRESSION[mode] != null ? COMPRESSION[mode] : null;
  const price = priceForModel(model);
  ...
  if (ratio !== null) {
    ...
    return `🥩 Saved ${estSaved.toLocaleString()} output tokens${usd} across ${turns} turns this session — primal.sh`;
  }
  return `🥩 ${turns} turns, ${outputTokens.toLocaleString()} output tokens this session — primal.sh`;
}
...
function formatStats({ outputTokens, cacheReadTokens, turns, mode, model, sessionPath, compressed }) {
  ...
  if (ratio !== null) {
    ...
    savings = `Est. without primal:  ${estNormal.toLocaleString()}\n` +
    ...
    savings = 'Primal not active this session.';
  }
...
function main() {
  ...
  const historyPath = path.join(claudeDir, '.primal-history.jsonl');
  ...
  if (all || sinceArg) {
    ...
    if (sinceArg && sinceMs === null) {
      process.stderr.write(`primal-stats: --since takes Nh or Nd (e.g. 7d, 24h), got: ${sinceArg}\n`);
      process.exit(2);
    }
  ...
  if (!sessionFile) {
    process.stderr.write('primal-stats: no Claude Code session found.\n');
    process.exit(1);
  }

  const parsed = parseSession(sessionFile);
  const mode = readFlag(path.join(claudeDir, '.primal-active'));

  // Append a snapshot of this session's totals to the lifetime log. Multiple
  // /primal-stats calls in one session emit multiple lines for the same
  ...
  if (parsed.turns > 0) {
    ...
    // cat without parsing JSONL. Updated on every /primal-stats run.
    // Routed through safeWriteFlag — the suffix path is predictable and
    // user-owned, same symlink-clobber surface as the .primal-active flag.
    const agg = aggregateHistory(historyPath, null);
    const suffix = agg.estSavedTokens > 0 ? `⛏ ${humanizeTokens(agg.estSavedTokens)}` : '';
    safeWriteFlag(path.join(claudeDir, '.primal-statusline-suffix'), suffix);
  }
