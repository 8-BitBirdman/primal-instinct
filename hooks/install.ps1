# primal — one-command hook installer for Claude Code (Windows PowerShell)
...
#        irm https://raw.githubusercontent.com/8-BitBirdman/primal-instinct/main/hooks/install.ps1 | iex
...
if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
    Write-Host "ERROR: 'node' is required to install the primal hooks (used to merge" -ForegroundColor Red
...
$RepoUrl = "https://raw.githubusercontent.com/8-BitBirdman/primal-instinct/main/hooks"
...
if (-not $Force) {
    ...
    if ($AllFilesPresent -and (Test-Path $Settings)) {
        try {
                ...
                foreach ($entry in $entries) {
                    if ($entry.hooks) {
                        foreach ($hookDef in $entry.hooks) {
                            if ($hookDef.command -and $hookDef.command.Contains("primal")) {
                                return $true
                            }
                        }
                    }
            ...
            $HooksWired = (& $hasPrimalHook "SessionStart") -and (& $hasPrimalHook "UserPromptSubmit")
    ...
    if ($AllFilesPresent -and $HooksWired -and $HasStatusLine) {
        Write-Host "Primal hooks already installed in $HooksDir"
...
if ($Force -and (Test-Path (Join-Path $HooksDir "primal-activate.js"))) {
    Write-Host "Reinstalling primal hooks (-Force)..."
} else {
    Write-Host "Installing primal hooks..."
}
...
$tmpScript = Join-Path $env:TEMP "primal-install-$([System.Diagnostics.Process]::GetCurrentProcess().Id).js"
try {
...
Write-Host "  - SessionStart hook: auto-loads primal rules every session"
Write-Host "  - Mode tracker hook: updates statusline badge when you switch modes"
Write-Host "    (/primal lite, /primal ultra, /primal-commit, etc.)"
Write-Host "  - Statusline badge: shows [PRIMAL] or [PRIMAL:ULTRA] etc."
