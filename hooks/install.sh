#!/bin/bash
# primal — one-command hook installer for Claude Code
# Installs: SessionStart hook (auto-load rules) + UserPromptSubmit hook (mode tracking)
# Usage: bash hooks/install.sh
#   or:  bash <(curl -s https://raw.githubusercontent.com/8-BitBirdman/primal-instinct/main/hooks/install.sh)
...
if ! command -v node >/dev/null 2>&1; then
  echo "ERROR: 'node' is required to install the primal hooks (used to merge"
...
REPO_URL="https://raw.githubusercontent.com/8-BitBirdman/primal-instinct/main/hooks"

HOOK_FILES=("package.json" "primal-config.js" "primal-activate.js" "primal-mode-tracker.js" "primal-stats.js" "primal-statusline.sh")
...
if [ "$FORCE" -eq 0 ]; then
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ -f "$SETTINGS" ]; then
    if PRIMAL_SETTINGS="$SETTINGS" node -e "
      const fs = require('fs');
      const settings = JSON.parse(fs.readFileSync(process.env.PRIMAL_SETTINGS, 'utf8'));
      const hasPrimalHook = (event) =>
        Array.isArray(settings.hooks?.[event]) &&
        settings.hooks[event].some(e =>
          e.hooks && e.hooks.some(h => h.command && h.command.includes('primal'))
        );
      process.exit(
        hasPrimalHook('SessionStart') &&
        hasPrimalHook('UserPromptSubmit') &&
        !!settings.statusLine
          ? 0
          : 1
      );
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ "$HOOKS_WIRED" -eq 1 ] && [ "$HAS_STATUSLINE" -eq 1 ]; then
    ALREADY_INSTALLED=1
    echo "Primal hooks already installed in $HOOKS_DIR"
...
if [ "$FORCE" -eq 1 ] && [ -f "$HOOKS_DIR/primal-activate.js" ]; then
  echo "Reinstalling primal hooks (--force)..."
else
  echo "Installing primal hooks..."
fi
...
chmod +x "$HOOKS_DIR/primal-statusline.sh"
...
PRIMAL_SETTINGS="$SETTINGS" PRIMAL_HOOKS_DIR="$HOOKS_DIR" node -e "
  const fs = require('fs');
  const settingsPath = process.env.PRIMAL_SETTINGS;
  const hooksDir = process.env.PRIMAL_HOOKS_DIR;
  const managedStatusLinePath = hooksDir + '/primal-statusline.sh';
  const settings = JSON.parse(fs.readFileSync(settingsPath, 'utf8'));
  if (!settings.hooks) settings.hooks = {};

  // SessionStart — auto-load primal rules
  ...
  if (!hasStart) {
    settings.hooks.SessionStart.push({
      hooks: [{
        type: 'command',
        command: 'node \"' + hooksDir + '/primal-activate.js\"',
        timeout: 5,
        statusMessage: 'Loading primal mode...'
      }]
    });
  }

  // UserPromptSubmit — track mode changes when user types /primal commands
  ...
  if (!hasPrompt) {
    settings.hooks.UserPromptSubmit.push({
      hooks: [{
        type: 'command',
        command: 'node \"' + hooksDir + '/primal-mode-tracker.js\"',
        timeout: 5,
        statusMessage: 'Tracking primal mode...'
      }]
    });
  }

  // Statusline — wire primal badge (report if skipped)
  if (!settings.statusLine) {
    ...
    if (cmd.includes(managedStatusLinePath)) {
      ...
      console.log('  NOTE: Existing statusline detected — primal badge NOT added.');
...
echo "  - SessionStart hook: auto-loads primal rules every session"
echo "  - Mode tracker hook: updates statusline badge when you switch modes"
echo "    (/primal lite, /primal ultra, /primal-commit, etc.)"
echo "  - Statusline badge: shows [PRIMAL] or [PRIMAL:ULTRA] etc."
