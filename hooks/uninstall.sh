#!/bin/bash
# primal — uninstaller for the SessionStart + UserPromptSubmit hooks
# Removes: hook files in ~/.claude/hooks, settings.json entries, and the flag file
# Usage: bash hooks/uninstall.sh
#   or:  bash <(curl -s https://raw.githubusercontent.com/8-BitBirdman/primal-instinct/main/hooks/uninstall.sh)
set -e

CLAUDE_DIR="${CLAUDE_CONFIG_DIR:-$HOME/.claude}"
HOOKS_DIR="$CLAUDE_DIR/hooks"
SETTINGS="$CLAUDE_DIR/settings.json"
FLAG_FILE="$CLAUDE_DIR/.primal-active"

HOOK_FILES=("package.json" "primal-config.js" "primal-activate.js" "primal-mode-tracker.js" "primal-stats.js" "primal-statusline.sh")

# Detect if primal is installed as a plugin (check plugin cache)
PLUGIN_INSTALLED=0
if [ -d "$CLAUDE_DIR/plugins" ]; then
  if find "$CLAUDE_DIR/plugins" -path "*/primal*" -name "plugin.json" -print -quit 2>/dev/null | grep -q .; then
    PLUGIN_INSTALLED=1
  fi
fi

if [ "$PLUGIN_INSTALLED" -eq 1 ]; then
  echo "Primal appears to be installed as a Claude Code plugin."
  echo "To uninstall the plugin, run:"
  echo ""
  echo "  claude plugin disable primal"
  echo ""
...
echo "Uninstalling primal hooks..."
...
if [ -f "$SETTINGS" ]; then
  ...
  if ! command -v node >/dev/null 2>&1; then
    ...
    echo "         Remove the primal SessionStart and UserPromptSubmit"
    ...
    PRIMAL_SETTINGS="$SETTINGS" PRIMAL_HOOKS_DIR="$HOOKS_DIR" node -e "
      ...
      // Remove statusLine if it references primal
...
echo ""
echo "Other agents:"
echo "  npx skills remove primal    # Cursor, Windsurf, Cline, Copilot, etc."
echo "  claude plugin disable primal  # Claude Code plugin"
echo "  gemini extensions uninstall primal  # Gemini CLI"
