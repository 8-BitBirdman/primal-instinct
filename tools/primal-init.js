#!/usr/bin/env node
// primal init — drop the always-on primal activation rule into a target
// repo for every IDE agent we support. Idempotent. Safe to re-run.
//
// Usage:
//   node tools/primal-init.js [target-dir] [--dry-run] [--force] [--only <agent>]
//   curl -fsSL https://raw.githubusercontent.com/8-BitBirdman/primal-instinct/main/tools/primal-init.js | node - [args]
//
...
// existing memory files — that's the job of `/primal:compress`.

const fs = require('fs');
const path = require('path');

// Embedded so the tool works standalone (npx-style) without the rules/ dir.
// Mirrors rules/primal-activate.md verbatim — keep these in sync.
const RULE_BODY = `Respond terse like smart primal. All technical substance stay. Only fluff die.
...
Switch instinct: /primal lite|full|ultra|wenyan
Stop: "stop primal" or "normal mode"

Auto-Clarity: drop primal for security warnings, irreversible actions, user confused. Resume after.

Boundaries: code/commits/PRs written normal.
`;

const SENTINEL = 'Respond terse like smart primal';

const AGENTS = [
  { id: 'cursor',   file: '.cursor/rules/primal.mdc',
    frontmatter: '---\ndescription: "Primal mode — terse communication, ~75% fewer tokens, full technical accuracy"\nalwaysApply: true\n---\n\n',
    mode: 'replace' },
  { id: 'windsurf', file: '.windsurf/rules/primal.md',
    frontmatter: '---\ntrigger: always_on\n---\n\n',
    mode: 'replace' },
  { id: 'cline',    file: '.clinerules/primal.md',
    frontmatter: '',
    mode: 'replace' },
...
function loadRuleBody() {
  ...
  try {
    const local = path.join(__dirname, '..', 'rules', 'primal-activate.md');
...
function help() {
  console.log(`primal init — drop always-on primal rule into a target repo

Usage: primal-init.js [target-dir] [--dry-run] [--force] [--only <agent>]
...
function main() {
  ...
  console.log(`🥩 primal init — ${opts.target}${opts.dryRun ? ' (dry run)' : ''}\n`);
