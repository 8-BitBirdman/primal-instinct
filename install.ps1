# primal — smart multi-agent installer (Windows / PowerShell).
#
# One line:
#   irm https://raw.githubusercontent.com/8-BitBirdman/primal-instinct/main/install.ps1 | iex
#
...
$InitScriptUrl = "$RawBase/tools/primal-init.js"
$McpShrinkPkg = "primal-shrink"

# ── Help ────────────────────────────────────────────────────────────────────
if ($Help) {
@"
primal installer (Windows) — detects your agents and installs primal for each.
...
FLAGS
  ...
  -WithMcpShrink   Claude Code: register primal-shrink MCP proxy. ON by default.
...
$Providers = @(
  ...
  @{ id='primal'; label='Google Primal'; profile='primal';  detect="dir:$HOME\.gemini\primal"; soft=1 }
)
...
function Install-Claude {
  ...
  if (-not $Force) {
    try {
      ...
      if ($list -match "(?i)primal") { $alreadyInstalled = $true }
    } catch {}
  }
  if ($alreadyInstalled) {
    ...
    if ((Try-Run "claude" @("plugin", "marketplace", "add", $Repo)) -and
        (Try-Run "claude" @("plugin", "install", "primal@primal"))) {
    ...
    if ($DryRun) {
      ...
      try {
        if ($localPs1) {
          ...
          $tmp = Join-Path $env:TEMP "primal-hooks-install-$([Guid]::NewGuid()).ps1"
  ...
  # registering an `npx -y primal-shrink` entry that 404s on every spawn.
  if ($WithMcpShrink) {
    Say "  → wiring primal-shrink MCP proxy (-WithMcpShrink)"
    if (Has-Cmd "npm") {
      $packageOnNpm = $false
      ...
      if (-not $packageOnNpm) {
        ...
        Record-Skipped "primal-shrink" "npm registry probe failed"
        Write-Host ""
        return
      }
    }
    ...
    if ($hasMcpAdd) {
      if ($DryRun) {
        Note "    would run: claude mcp add primal-shrink -- npx -y $McpShrinkPkg"
      } else {
        if (Try-Run "claude" @("mcp", "add", "primal-shrink", "--", "npx", "-y", $McpShrinkPkg)) {
          Record-Installed "primal-shrink"
          ...
          Record-Failed "primal-shrink" "claude mcp add failed"
        }
      }
    } else {
      ...
      Write-Host '    }'
      Write-Host ""
      Record-Skipped "primal-shrink" "manual config required (snippet printed)"
    }
  }
  Write-Host ""
}
...
function Run-Init {
  ...
  if ($RepoRoot -and (Test-Path (Join-Path $RepoRoot "tools\primal-init.js")) -and (Has-Cmd "node")) {
    if (Try-Run "node" (@((Join-Path $RepoRoot "tools\primal-init.js")) + $initArgs)) { return $true }
    return $false
  }
  ...
  $tmp = Join-Path $env:TEMP "primal-init-$([Guid]::NewGuid()).js"
  try {
...
if ($WithInit) {
  ...
  if (Run-Init) {
    Record-Installed "primal-init ($PWD)"
  } else {
    Record-Failed "primal-init" "tools/primal-init.js failed"
  }
  Write-Host ""
