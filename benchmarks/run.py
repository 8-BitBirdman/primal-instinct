#!/usr/bin/env python3
"""Benchmark primal vs normal Claude output token counts."""
...
SKILL_PATH = REPO_DIR / "skills" / "primal" / "SKILL.md"
...
def run_benchmarks(client, model, prompts, primal_system, trials):
    results = []
    total = len(prompts)
    ...
    for i, prompt_entry in enumerate(prompts, 1):
        ...
        for mode, system in [("normal", NORMAL_SYSTEM), ("primal", primal_system)]:
...
def compute_stats(results):
    ...
    for entry in results:
        ...
        primal_medians = statistics.median(
            [t["output_tokens"] for t in entry["primal"]]
        )
        savings = 1 - (primal_medians / normal_medians) if normal_medians > 0 else 0
        ...
        rows.append(
            {
                ...
                "primal_median": int(primal_medians),
    ...
    avg_primal = round(statistics.mean([r["primal_median"] for r in rows]))
...
def format_table(rows, summary):
    lines = [
        "| Task | Normal (tokens) | Primal (tokens) | Saved |",
        "|------|---------------:|----------------:|------:|",
    ]
    for r in rows:
        label = format_prompt_label(r["id"])
        lines.append(
            f"| {label} | {r['normal_median']} | {r['primal_median']} | {r['savings_pct']}% |"
        )
    lines.append(
        f"| **Average** | **{summary['avg_normal']}** | **{summary['avg_primal']}** | **{summary['avg_savings']}%** |"
    )
    lines.append("")
    lines.append(
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark primal vs normal Claude")
    ...
    primal_system = load_primal_system()
    ...
    results = run_benchmarks(client, args.model, prompts, primal_system, args.trials)
