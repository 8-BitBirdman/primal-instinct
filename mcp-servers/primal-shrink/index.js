#!/usr/bin/env node
// primal-shrink — MCP middleware that proxies an upstream MCP server and
// compresses prose fields so the model sees fewer tokens.
//
// Usage:
//   primal-shrink <upstream-command> [...args]
//
// Example wrapping the filesystem MCP server:
//   "mcpServers": {
//     "fs-shrunk": {
//       "command": "npx",
//       "args": ["primal-shrink", "npx", "@modelcontextprotocol/server-filesystem", "/some/path"]
//     }
//   }
//
// Compression is applied to:
//   - "description" fields in tools/list, prompts/list, resources/list responses
//   - same boundaries as primal-compress: code, URLs, paths, identifiers preserved
//
...
//   PRIMAL_SHRINK_FIELDS   comma-separated extra field names to compress
//                           (default: description)
//   PRIMAL_SHRINK_DEBUG=1  log compression deltas to stderr
...
if (args.length === 0) {
  process.stderr.write('primal-shrink: missing upstream command.\n');
  process.stderr.write('Usage: primal-shrink <upstream-command> [...args]\n');
  process.exit(2);
}

const debug = process.env.PRIMAL_SHRINK_DEBUG === '1';
const fields = (process.env.PRIMAL_SHRINK_FIELDS || 'description')
...
upstream.on('error', err => {
  process.stderr.write(`primal-shrink: failed to spawn upstream: ${err.message}\n`);
  process.exit(1);
});
            ...
            if (out !== before) {
              ...
              if (debug) {
                process.stderr.write(
                  `[primal-shrink] ${arrayName}.${item.name || '?'}.${field}: ` +
